...
import modules.filework
import modules.compress_image
import modules.pdf_docx_func
...
def pdf2docx(choice, docs):
    ...
    try:
        modules.pdf_docx_func.pdf_to_docx(choice, docs)
        sg.popup('Успешно!')
...
def docx2pdf(choice, docs):
    ...
    try:
        modules.pdf_docx_func.docx_to_pdf(choice, docs)
        sg.popup('Успешно!')
...
def compressed_image(choice, images, compression):
    ...
    try:
        modules.compress_image.compress_img(choice, images, compression)
...
def deleteFiles(podstr, type):
    ...
    try:
        if modules.filework.find_files(podstr, type=type) != {}:
            modules.filework.delete_files(str(type), podstr)
            sg.popup('Успешно!')
...
layout_left = [
    [sg.Text(f"Рабочий каталог: "), sg.Text(modules.filework.return_curr_dir(), key='dirtext'), sg.Button('Выбрать каталог')],
...
main_window['file_dir'].update(modules.filework.os.listdir())
flag = 0
...
while True:
    ...
    if window == main_window and event in (sg.WINDOW_CLOSED, 'Выход'):
        ...
        try:
            modules.filework.os.chdir(sg.popup_get_folder('Выберите папку', title='Выбор папки'))
            main_window['dirtext'].update(modules.filework.return_curr_dir())
        ...
        if flag == 1:
            ...
            if values[0]:
                pdf2docx('0', modules.filework.find_files('.pdf', type=0))
            ...
            if values[0]:
                docx2pdf('0', modules.filework.find_files('.docx', type=0))
            ...
            if values[0]:
                compressed_image('0', modules.filework.find_files('.jpg', '.jpeg', '.gif', '.png', type=0), values['compression'])
